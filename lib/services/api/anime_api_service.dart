...
class AnimeApiService {
  ...
  static bool _isSameAnime(String inputAnimeName, String apiAnimeTitle) {
    ...
    final inputWords =
        cleanInput.split(' ').where((w) => w.length > 2).toList();
    final titleWords =
    ...
    final commonWords =
        inputWords.where((w) => titleWords.contains(w)).length;
  ...
  static Future<List<Map<String, dynamic>>> getAnimeFranchiseFullDetails(
    ...
    try {
      ...
      final searchUrl = Uri.parse('$_baseUrl/anime')
          .replace(queryParameters: {'q': searchKey, 'limit': '15'});
  ...
  static Future<bool> validateAnimeExists(String animeName) async {
    try {
      final url = Uri.parse('$_baseUrl/anime')
          .replace(queryParameters: {'q': animeName, 'limit': '1'});
      final response = await http
          .get(url, headers: _headers)
          .timeout(const Duration(seconds: 10));
  ...
  static Future<bool> isCharacterInFranchise(
      {required String animeName, required String characterName}) async {
    try {
      final url = Uri.parse('$_baseUrl/characters').replace(
          queryParameters: {'q': characterName, 'limit': '15'});
      final response = await http
          .get(url, headers: _headers)
          .timeout(const Duration(seconds: 10));
      if (response.statusCode != 200) return false;
  ...
  // ── إذا animeIds موجودة → بحث داخل الأنمي المحدد
  // ── إذا animeIds فارغة أو null → بحث عالمي مرتب حسب الشهرة
  // =========================================================
