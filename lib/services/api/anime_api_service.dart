...
class AnimeApiService {
  ...
  static Future<List<Map<String, dynamic>>> getAnimeFranchiseFullDetails(int malId, String animeName) async {
    try {
      ...
      final searchUrl = Uri.parse('$_baseUrl/anime').replace(queryParameters: {'q': searchKey, 'limit': '15'});
        ...
        for (var res in results) {
          ...
          if (!processedIds.contains(resId) && resTitle.contains(sanitizedBaseName)) {
            ...
            franchiseParts.add({
              'id': resId,
              'title': res['title'],
              'image_url': res['images']?['jpg']?['large_image_url'] ?? res['images']?['jpg']?['image_url'],
            });
          }
        }
      }
  ...
  static Future<bool> validateAnimeExists(String animeName) async {
    try {
      final url = Uri.parse('$_baseUrl/anime').replace(queryParameters: {'q': animeName, 'limit': '1'});
      final response = await http.get(url, headers: _headers).timeout(const Duration(seconds: 10));
          ...
          for (final item in data['data']) {
            ...
            if (sanitizedApiName.contains(cleanInput) || cleanInput.contains(sanitizedApiName)) {
              if (!addedNames.contains(apiName)) {
                ...
                results.add({'name': apiName, 'imageUrl': character['images']?['jpg']?['image_url'] ?? ''});
              }
            }
          }
        } catch (e) {
