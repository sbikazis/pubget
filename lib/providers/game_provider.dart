...
class GameProvider extends ChangeNotifier {
  ...
  Future<Map<String, String>?> createGame({
    ...
    try {
      ...
      if (!GameLogicValidator.canCreateNewGame(activeGames)) {
        throw Exception(
          "المجموعة ممتلئة، هناك لعبتان قيد التنفيذ حالياً.",
        );
      }

      String assignedSlot = activeGames.any(
        (g) => g.gameSlot == 'game_1',
      )
          ? 'game_2'
          : 'game_1';
  ...
  Future<bool> submitChainWord({
    ...
    return FirebaseFirestore.instance.runTransaction((tx) async {
      ...
      if (!GameLogicValidator.isUserTurn(
        userId,
      ...
      if (!GameLogicValidator.isValidChainWord(
        cleanWord,
        game.lastLetter,
        game.usedWords,
      )) {
      ...
      tx.update(ref, {
        ...
        'lastLetter':
            cleanWord.substring(cleanWord.length - 1),
      ...
      tx.set(
        FirebaseFirestore.instance
            .collection(
              FirestorePaths.groupMessages(groupId),
            )
            .doc(messageId),
        {
          'id': messageId,
  ...
  Future<String> joinGame({
    ...
    await FirebaseFirestore.instance
      ...
      if (!snapshot.exists) {
        throw Exception("اللعبة لم تعد موجودة!");
      }
      ...
      if (game.playerTwoId != null &&
        ...
        throw Exception(
          "آسفون! قام شخص آخر بالانضمام لهذه اللعبة قبلك.",
        );
      }
      ...
      if (game.status !=
        ...
        throw Exception(
          "هذه اللعبة بدأت بالفعل أو لم تعد متاحة للانضمام.",
        );
      }
    ...
    if (startWordToAnnounce != null) {
      await _sendGameSystemMessage(
        ...
        gameSlot: 'chain',
        text:
            '🔗 بدأت سلسلة الأنمي! الكلمة الأولى: $startWordToAnnounce',
      );
    }
  ...
  Future<void> setCharacter({
    ...
    final charData = (validatedName != null &&
    ...
    if (charData == null) {
      throw Exception("هذه الشخصية غير موجودة.");
    }
    ...
    return FirebaseFirestore.instance
      ...
      if (game.setupStartedAt != null &&
          GameTimerManager.hasSetupTimeout(
            game.setupStartedAt!,
          )) {
        throw Exception(
          "انتهى الوقت المحدد للاختيار (60 ثانية)!",
        );
      }
      ...
      if (userId == game.playerOneId) {
        updates['playerOneCharacter'] =
            charData['name'];

        updates['playerOneImage'] =
            charData['imageUrl'];
      } else {
        updates['playerTwoCharacter'] =
            charData['name'];

        updates['playerTwoImage'] =
            charData['imageUrl'];
      }
  ...
  Future<bool> guessCharacter({
    ...
    if (GameLogicValidator.isGuessCorrect(
      guessedName,
      opponentChar ?? "",
    )) {
      await _sendGameSystemMessage(
        ...
        gameSlot: game.gameSlot,
        text:
            "✅ ${userName ?? 'لاعب'} خمّن '$guessedName' وهي صحيحة!",
      );

      final rewarded = await finishGame(
        groupId,
        gameId,
        winnerId: userId,
        winnerName: userName,
      ...
      await _sendGameSystemMessage(
        ...
        gameSlot: game.gameSlot,
        text:
            "❌ ${userName ?? 'لاعب'} خمن '$guessedName' وهي خاطئة!",
      );
  ...
  Future<bool> finishGame(
    ...
    // ✅ إصلاح المكافأة والتحديث الفوري
    if (!isCancelled && winnerId != null) {
      try {
        rewarded = await _coinService
            .rewardEventWin(
          userId: winnerId,
          gameId: gameId,
        );
        ...
        if (rewarded) {
          ...
          debugPrint(
            '✅ تمت إضافة +10 عملات وتحديث الرصيد فوراً',
          );
        } else {
          debugPrint(
            '⚠️ وصل المستخدم للحد اليومي (3/اليوم)',
          );
        }
      } catch (e) {
        debugPrint(
          '❌ Failed to reward coins: $e',
        );
      }
    }

    String resolvedLoserName =
        loserName ?? "الخصم";
  ...
  Stream<List<GameModel>> streamActiveGames(
    String groupId,
  ) {
    return FirebaseFirestore.instance
        .collection(
          FirestorePaths.groupGames(groupId),
        )
        .where(
          'status',
          whereIn: [
  ...
  Stream<GameModel?> streamCurrentGame(
    String groupId,
    String gameId,
  ) {
    return _firestore
  ...
  Future<void> processAutoJudge(
    ...
    final timeoutType =
        GameAutoJudge.checkTimeout(game);
    ...
    String? timedOutPlayerId =
        GameAutoJudge.getTimedOutPlayerId(
      game,
    );

    if (timedOutPlayerId == null &&
        game.gameType == 'anime_chain') {
      timedOutPlayerId =
          game.currentTurnUserId;
    }

    String reason =
        GameAutoJudge.getReasonMessage(
      timeoutType,
      timedOutPlayerId ?? "مجهول",
    );

    if (timeoutType ==
        TimeoutType.totalGameTimeout) {
      await finishGame(
        groupId,
        game.id,
        isCancelled: false,
      ...
      await finishGame(
        ...
        reason: game.gameType ==
                'anime_chain'
            ? "تأخر في إرسال الإسم وانتهى الوقت"
            : reason,
        loserName: loserName,
      );
    }
