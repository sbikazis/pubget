...
class _UploadEditScreenState extends State<UploadEditScreen> {
  ...
  Future<void> _pickVideo() async {
    ...
    try {
      ...
      if (picked == null) { _finishPreparing(); return; }
      ...
      final thumbnailPath = await VideoThumbnail.thumbnailFile(video: picked.path, thumbnailPath: tempDir.path, imageFormat: ImageFormat.JPEG, quality: 75);
  ...
  Future<void> _submitUpload() async {
    ...
    if (animeTitle.isEmpty) { _showError('اكتب اسم الأنمي'); return; }
  ...
  Widget _buildVideoPreview() {
    if (_isPreparingVideo) return const Center(child: CircularProgressIndicator());
    if (_videoInitialized && _videoController!= null) {
      return ClipRRect(
        ...
        child: Stack(alignment: Alignment.center, children: [
          ...
          Container(padding: const EdgeInsets.all(12), decoration: const BoxDecoration(color: Colors.black45, shape: BoxShape.circle), child: Icon(_videoController!.value.isPlaying? Icons.pause : Icons.play_arrow, color: Colors.white, size: 36)),
          Positioned(top: 10, right: 10, child: GestureDetector(onTap: _pickVideo, child: Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6), decoration: BoxDecoration(color: Colors.black54, borderRadius: BorderRadius.circular(20)), child: const Text('تغيير', style: TextStyle(color: Colors.white, fontSize: 12))))),
          Positioned(bottom: 0, left: 0, right: 0, child: VideoProgressIndicator(_videoController!, allowScrubbing: true, colors: const VideoProgressColors(playedColor: Colors.white, bufferedColor: Colors.white38, backgroundColor: Colors.white12))),
        ]),
      );
    }
    return const Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      Icon(Icons.video_library, color: Colors.grey, size: 50),
      SizedBox(height: 8),
      Text('اضغط لاختيار فيديو', style: TextStyle(color: Colors.grey)),
      SizedBox(height: 4),
      Text('الحد الأقصى 60 ثانية • 50MB', style: TextStyle(color: Colors.grey, fontSize: 12)),
    ]);
  }
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const Text('الوصف (اختياري)', style: TextStyle(fontWeight: FontWeight.bold)),
          const SizedBox(height: 8),
          TextField(controller: _captionController, maxLines: 3, maxLength: 150, decoration: InputDecoration(hintText: 'اكتب شيئاً عن الإيديت...', border: OutlineInputBorder(borderRadius: BorderRadius.circular(10)))),
        ]),
      ),
    );
  }
