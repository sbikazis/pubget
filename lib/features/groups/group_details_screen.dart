...
class _GroupDetailsScreenState extends State<GroupDetailsScreen> {
  ...
  Future<void> _processJoin(
      GroupModel group, UserModel user, String? inviterName) async {
  ...
  Widget _buildActions(GroupModel group, List<MemberModel> members) {
    ...
    final currentMember =
        isMember ? members.firstWhere((m) => m.userId == user.id) : null;
  ...
  Future<void> _handleLeaveGroup(
      GroupModel group, MemberModel member) async {
    Navigator.of(context).pop();
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            const Text(
                'سيتم حذف المجموعة نهائياً وإشعار جميع الأعضاء. يمكنك كتابة رسالة وداع لهم:'),
