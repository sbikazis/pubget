# PUBGET 1.0 — Product & System Master Specification

**الحالة:** LOCKED — مرجع البناء الرسمي المقفَل  
...
هذه الوثيقة هي المرجع الرسمي الوحيد للمنتج والنظام في Pubget 1.0.
لا تُعامل كـ«أفكار عامة». كل قرار بناء لاحق يُقاس عليها.

ترتيب السلطة عند التعارض:

1. هذه الوثيقة (Pubget 1.0 Master Specification)
2. قرارات المنتج المقفلة داخلها (§122)
3. قيود الأمان والموثوقية — لا تُضعَف أبدًا
4. قيود المعمارية (UI → Provider → Repository → Firebase)
5. مستند التنفيذ البصري/الهندسي هو **طريقة تنفيذ** وليس مصدر منتج بديل
6. إذا بقي تعارض حقيقي: يُوثَّق ولا يُختلق قرار منتج

ما يُمنع صراحة أثناء التنفيذ:

- Prompt 18
- إرسال أي شيء إلى Replit
- اختراع backend أو أزرار ميتة أو بيانات وهمية
- إضعاف قواعد Firestore/Storage
- حذف بيانات إنتاج دون مسار ترحيل موثّق
- إعلان Definition of Done قبل اكتمال UI + منطق + بيانات + أمان + حالات + اختبارات

التصنيف الحي مقابل النسخة الموجودة: `docs/PUBGET_1_0_REBUILD_MATRIX.md`.

---

PUBGET 1.0
الوثيقة المرجعية النهائية للنسخة الجديدة
الحالة: Master Specification / مرجع البناء — LOCKED
المنتج: Pubget
الهوية: منصة اجتماعية متخصصة لعشاق الأنمي والمجتمعات
المنصات الحالية: Android
اللغات: العربية + الإنجليزية
...
124. علاقة وثيقة التنفيذ البصري بهذه المواصفة
مستند NEXT-GENERATION PRODUCT REBUILD هو خطة تنفيذ هندسية/بصرية.
لا يضيف ميزات منتج جديدة.
لا يتجاوز §110–§113 و§122.
لا يُعلن اكتمال المنتج لمجرد وجود شاشات أو نجاح الاختبارات.
المرحلة الحالية بعد القفل: مقارنة حيّة ثم FIX / REBUILD / DELETE / KEEP / BLOCKED في `docs/PUBGET_1_0_REBUILD_MATRIX.md`.
